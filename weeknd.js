...
const NEWS_APIS = [
    // GNews API with your API key (most reliable for GitHub Pages)
    `https://gnews.io/api/v4/search?q=The%20Weeknd&lang=en&country=us&max=30&apikey=8613a856b2602921c5ba28eda6e22483`,
    // Alternative RSS feeds that work better with GitHub Pages
    'https://api.rss2json.com/v1/api.json?rss_url=https://www.rollingstone.com/feed/',
    'https://api.rss2json.com/v1/api.json?rss_url=https://www.nme.com/feed/news',
    'https://api.rss2json.com/v1/api.json?rss_url=https://www.billboard.com/feed/rss/news',
    // NewsAPI with different CORS proxy
    `https://api.allorigins.win/raw?url=${encodeURIComponent(`https://newsapi.org/v2/everything?q=The%20Weeknd&sortBy=publishedAt&language=en&pageSize=30&apiKey=${NEWS_API_KEY}`)}`
];

function isDirectlyAboutWeeknd(article) {
    const keyword = /the weeknd|abel tesfaye|starboy|after hours|dawn fm|blinding lights/i;
    // Check if "The Weeknd", "Abel Tesfaye", or his albums/songs are in the title or description
    return keyword.test(article.title) || (article.description && keyword.test(article.description.slice(0, 500)));
}
